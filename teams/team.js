...
function renderCurrentSeason(teamName, team) {
  ...
  const formHtml = season.played >= 5
    ? `
      <div class="team-form">
        ${season.form.map(result =>
          `<span class="form-result form-${result.toLowerCase()}">
            ${result}
          </span>`
        ).join("")}
      </div>
    `
    : `
      <p class="team-form-pending">
        Form will appear after 5 matches have been played.
      </p>
    `;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  page.innerHTML = `
    <section class="page-title team-title">
      ...
      ${team.established ? `<p class="team-established">Est. ${escapeHtml(team.established)}</p>` : ""}
    </section>
